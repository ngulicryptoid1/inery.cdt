...
Error WasmObjectFile::parseIneryABISection(ReadContext& Ctx) {
   StringRef sr = readString(Ctx);
   inery_abi = sr;

   if (Ctx.Ptr != Ctx.End)
      return make_error<GenericBinaryError>("inery abi section ended prematurely",
                                          object_error::parse_failed);
   return Error::success();
...
Error WasmObjectFile::parseCustomSection(WasmSection &Sec, ReadContext &Ctx) {
  if (Sec.Name == ".imports") {
     if (Error Err = parseAllowedSection(Ctx))
        return Err;
  } else if (Sec.Name == ".inery_abi") {
     if (Error Err = parseIneryABISection(Ctx))
        return Err;
  } else if (Sec.Name == ".inery_actions") {
     if (Error Err = parseActionsSection(Ctx))
        return Err;
  } else if (Sec.Name == ".inery_notify") {
