...
void WebAssemblyAsmPrinter::EmitEndOfAsmFile(Module &M) {
  ...
  bool has_abi    = false;
  bool has_inery_action = false;
  bool has_inery_notify = false;
  for (const auto &F : M) {
     if (F.hasFnAttribute("inery_wasm_import"))
        has_import = true;
     if (F.hasFnAttribute("inery_wasm_abi"))
        has_abi = true;
     if (F.hasFnAttribute("inery_wasm_action"))
        has_inery_action = true;
     if (F.hasFnAttribute("inery_wasm_notify"))
        has_inery_notify = true;
  }
  ...
  if (has_abi) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".inery_abi";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("inery_wasm_abi")) {
           StringRef abi = F.getFnAttribute("inery_wasm_abi").getValueAsString();
  ...
  if (has_inery_action) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".inery_actions";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("inery_wasm_action")) {
           StringRef action_name = F.getFnAttribute("inery_wasm_action").getValueAsString();
  ...
  if (has_inery_notify) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".inery_notify";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("inery_wasm_notify")) {
           StringRef name = F.getFnAttribute("inery_wasm_notify").getValueAsString();
