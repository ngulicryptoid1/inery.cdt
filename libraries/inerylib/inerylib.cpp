...
#include "core/inery/powers.hpp"
#include "contracts/inery/system.hpp"
#include "contracts/inery/privileged.hpp"
...
namespace inery {
   ...
   bool block_signing_authority_v0::is_valid()const {
      ...
      std::set<inery::public_key> unique_keys;
   ...
   void set_blockchain_parameters(const inery::blockchain_parameters& params) {
      char buf[sizeof(inery::blockchain_parameters)];
   ...
   void get_blockchain_parameters(inery::blockchain_parameters& params) {
      ...
      inery::check( size <= sizeof(buf), "buffer is too small" );
      inery::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
