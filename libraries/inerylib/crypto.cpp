...
namespace inery {
   ...
   inery::public_key recover_key( const inery::checksum256& digest, const inery::signature& sig ) {
      ...
      inery::public_key pubkey;
      if ( pubkey_size <= sizeof(optimistic_pubkey_data) ) {
         inery::datastream<const char*> pubkey_ds( optimistic_pubkey_data, pubkey_size );
         pubkey_ds >> pubkey;
      } else {
         ...
         inery::datastream<const char*> pubkey_ds( reinterpret_cast<const char*>(pubkey_data), pubkey_size );
         pubkey_ds >> pubkey;
   ...
   void assert_recover_key( const inery::checksum256& digest, const inery::signature& sig, const inery::public_key& pubkey ) {
      ...
      auto sig_data = inery::pack(sig);
      auto pubkey_data = inery::pack(pubkey);
