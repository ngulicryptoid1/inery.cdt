...
namespace inery { namespace cdt {
   ...
   struct codegen_exception : public std::exception {
      virtual const char* what() const throw() {
         return "inery.codegen fatal error";
      }
   };
   ...
   // remove after v1.7.0
   bool has_inerylib = false;

   class inery_ppcallbacks : public PPCallbacks {
      public:
         inery_ppcallbacks(SourceManager& sm, std::string file) : sources(sm), fn(file) {}
      protected:
         virtual void InclusionDirective(
            ...
            if ( file_name.find("inerylib") != StringRef::npos )
               has_inerylib = true;
         }
   ...
   class inery_codegen_visitor : public RecursiveASTVisitor<inery_codegen_visitor>, public generation_utils {
      private:
      ...
      public:
         ...
         explicit inery_codegen_visitor(CompilerInstance *CI)
         ...
         bool is_datastream(const QualType& qt) {
            ...
            auto ds_re    = std::regex("(((class inery::)?datastream<[a-zA-Z]+[a-zA-Z0-9]*.*>)|(DataStream)) &");
         ...
         */
         ...
         void create_dispatch(const std::string& attr, const std::string& func_name, F&& get_str, CXXMethodDecl* decl) {
            ...
            if (cg.is_inery_contract(decl, cg.contract_name)) {
               if (has_inerylib) {
                  ss << "\n\n#include <inerylib/datastream.hpp>\n";
                  ss << "#include <inerylib/name.hpp>\n";
               } else {
                  ss << "\n\n#include <inery/datastream.hpp>\n";
                  ss << "#include <inery/name.hpp>\n";
               }
               ss << "extern \"C\" {\n";
               ...
               ss << "}\n";
               ss << "inery::datastream<const char*> ds{(char*)buff, as};\n";
               int i=0;
               ...
               ss << decl->getParent()->getQualifiedNameAsString() << "{inery::name{r},inery::name{c},ds}." << decl->getNameAsString() << "(";
         ...
         void create_action_dispatch(CXXMethodDecl* decl) {
            ...
            create_dispatch("inery_wasm_action", "__inery_action_", func, decl);
         }

         void create_notify_dispatch(CXXMethodDecl* decl) {
            auto func = [](CXXMethodDecl* d) { return generation_utils::get_notify_pair(d); };
            create_dispatch("inery_wasm_notify", "__inery_notify_", func, decl);
         }
         ...
         virtual bool VisitCXXMethodDecl(CXXMethodDecl* decl) {
            ...
            if (decl->isIneryAction()) {
               name = generation_utils::get_action_name(decl);
               validate_name(name, [&]() {emitError(*ci, decl->getLocation(), "action not a valid inery name");});
      ...
      class inery_codegen_consumer : public ASTConsumer {
      private:
         inery_codegen_visitor *visitor;
         std::string main_file;
         CompilerInstance* ci;

      public:
         explicit inery_codegen_consumer(CompilerInstance *CI, std::string file)
            : visitor(new inery_codegen_visitor(CI)), main_file(file), ci(CI) { }
         ...
         virtual void HandleTranslationUnit(ASTContext &Context) {
            ...
            if (main_fe) {
               ...
               try {
                  ...
                  std::stringstream ss;
                  ss << "extern \"C\" {\n";
                  ss << "void inery_assert_code(uint32_t, uint64_t);";
                  ss << "\t__attribute__((weak, inery_wasm_entry, inery_wasm_abi(";
                  std::string abi = cg.abi;
                  ss << "\"" << _quoted(abi) << "\"";
                  ss << ")))\n";
                  ss << "\tvoid __insert_inery_abi(unsigned long long r, unsigned long long c, unsigned long long a){";
                  ss << "inery_assert_code(false, 1);";
                  ss << "}\n";
                  ss << "}";
      ...
      class inery_codegen_frontend_action : public ASTFrontendAction {
      public:
         virtual std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI, StringRef file) {
            CI.getPreprocessor().addPPCallbacks(_make_unique<inery_ppcallbacks>(CI.getSourceManager(), file.str()));
            return _make_unique<inery_codegen_consumer>(&CI, file);
         }
   };
