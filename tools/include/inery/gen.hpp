...
struct generation_utils {
      ...
      auto check = [&](const clang::Type* pt) {
        if (auto tst = llvm::dyn_cast<clang::TemplateSpecializationType>(pt))
         if (auto rt = llvm::dyn_cast<clang::RecordType>(tst->desugar()))
            return rt->getDecl()->isIneryIgnore();
      ...
      auto get = [&](const clang::Type* pt) {
         if (auto tst = llvm::dyn_cast<clang::TemplateSpecializationType>(pt))
            if (auto decl = llvm::dyn_cast<clang::RecordType>(tst->desugar()))
               return decl->getDecl()->isIneryIgnore() ? tst->getArg(0).getAsType() : type;
         return type;
      };
   ...
   static inline bool has_inery_ricardian( const clang::CXXMethodDecl* decl ) {
      return decl->hasIneryRicardian();
   }
   static inline bool has_inery_ricardian( const clang::CXXRecordDecl* decl ) {
      return decl->hasIneryRicardian();
   }

   static inline std::string get_inery_ricardian( const clang::CXXMethodDecl* decl ) {
      return decl->getIneryRicardianAttr()->getName();
   }
   static inline std::string get_inery_ricardian( const clang::CXXRecordDecl* decl ) {
      return decl->getIneryRicardianAttr()->getName();
   }

   static inline std::string get_action_name( const clang::CXXMethodDecl* decl ) {
      std::string action_name = "";
      auto tmp = decl->getIneryActionAttr()->getName();
      if (!tmp.empty())
         return tmp;
   ...
   static inline std::string get_notify_pair( const clang::CXXMethodDecl* decl ) {
      ...
      auto tmp = decl->getIneryNotifyAttr()->getName();
      return tmp;
   }
   ...
   static inline bool is_inery_contract( const clang::CXXMethodDecl* decl, const std::string& cn ) {
      std::string name = "";
      if (decl->isIneryContract())
         name = decl->getIneryContractAttr()->getName();
      else if (decl->getParent()->isIneryContract())
         name = decl->getParent()->getIneryContractAttr()->getName();
      if (name.empty()) {
   ...
   static inline bool is_inery_contract( const clang::CXXRecordDecl* decl, const std::string& cn ) {
      std::string name = "";
      auto pd = llvm::dyn_cast<clang::CXXRecordDecl>(decl->getParent());
      if (decl->isIneryContract()) {
         auto nm = decl->getIneryContractAttr()->getName().str();
         name = nm.empty() ? decl->getName().str() : nm;
      }
      else if (pd && pd->isIneryContract()) {
         auto nm = pd->getIneryContractAttr()->getName().str();
