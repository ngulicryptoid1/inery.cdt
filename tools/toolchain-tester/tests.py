...
class Test(ABC):
    ...
    @abstractmethod
    def _run(self, inery_cpp: str, args: List[str]):
        pass
    ...
    def run(self):
        ...
        inery_cpp = os.path.join(Config.cdt_path, "inery-cpp")
        self._run(inery_cpp, args)
...
class BuildPassTest(Test):
    def _run(self, inery_cpp, args):
        command = [inery_cpp, self.cpp_file]
        command.extend(args)
...
class CompilePassTest(Test):
    def _run(self, inery_cpp, args):
        command = [inery_cpp, "-c", self.cpp_file]
        command.extend(args)
        res = subprocess.run(command, capture_output=True)
        self.handle_test_result(res)
...
class AbigenPassTest(Test):
    def _run(self, inery_cpp, args):
        command = [inery_cpp, self.cpp_file, "-abigen_output=''"]
        command.extend(args)
        res = subprocess.run(command, capture_output=True)
        self.handle_test_result(res)
