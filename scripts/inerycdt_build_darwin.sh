...
if [ "${OS_MIN}" -lt 12 ]; then
	echo "You must be running Mac OS 10.12.x or higher to install INERY."
	echo "Exiting now."
	exit 1
fi

if [ "${DISK_AVAIL}" -lt "$DISK_MIN" ]; then
	echo "You must have at least ${DISK_MIN}GB of available storage to install INERY."
	echo "Exiting now."
	exit 1
fi
...
then
	printf "Homebrew must be installed to compile INE.IO!\\n"
...
while read -r name tester testee brewname uri; do
	...
	(( COUNT++ ))
done < "${REPO_ROOT}/scripts/inerycdt_build_darwin_deps"
IFS="${var_ifs}"
