...
if [[ "${OS_NAME}" == "Amazon Linux AMI" && "${OS_VER}" -lt 2017 ]]; then
	printf "You must be running Amazon Linux 2017.09 or higher to install INERY.\\n"
	printf "exiting now.\\n"
	exit 1
fi
...
then
	printf "\\nYum must be installed to compile INE.IO.\\n"
	printf "\\nExiting now.\\n"
	exit 1
fi
...
printf "\\n"
if [ "${COUNT}" -gt 1 ]; then
	printf "The following dependencies are required to install INERY.\\n"
	printf "${DISPLAY}\\n\\n"
