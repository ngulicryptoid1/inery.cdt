...
#include <inery/abigen.hpp>
#include <inery/codegen.hpp>
...
#define CPP_COMP 1
#define COMPILER_NAME "inery-cpp"
...
namespace inery { namespace cdt {
   DeclarationMatcher function_decl_matcher = cxxMethodDecl().bind("inery_tool");
   DeclarationMatcher record_decl_matcher   = cxxRecordDecl().bind("inery_tool");
   DeclarationMatcher typedef_decl_matcher  = typedefDecl().bind("inery_tool");
   auto               class_tmp_matcher     = classTemplateSpecializationDecl().bind("inery_tool");
   ...
   class IneryMethodMatcher : public MatchFinder::MatchCallback {
      public:
         virtual void run( const MatchFinder::MatchResult& res ) {
            if (const clang::CXXMethodDecl* decl = res.Nodes.getNodeAs<clang::CXXMethodDecl>("inery_tool")->getCanonicalDecl()) {
               abi abi;
   ...
   class IneryRecordMatcher : public MatchFinder::MatchCallback {
      public:
         ...
         virtual void run( const MatchFinder::MatchResult& res ) {
            if (const clang::CXXRecordDecl* decl = res.Nodes.getNodeAs<clang::CXXRecordDecl>("inery_tool")) {
            ...
            if (const clang::ClassTemplateSpecializationDecl* decl = res.Nodes.getNodeAs<clang::ClassTemplateSpecializationDecl>("inery_tool")) {
...
void generate(const std::vector<std::string>& base_options, std::string input, std::string contract_name, const std::vector<std::string>& resource_paths, bool abigen) {
   ...
   CommonOptionsParser opts( size, new_argv, IneryCompilerToolCategory, 0 );
   ...
   tool_run = ctool.run(newFrontendActionFactory<inery_codegen_frontend_action>().get());
   if (tool_run != 0) {
...
int main(int argc, const char **argv) {
   ...
   for (int i=0; i < argc; i++) {
     if (argv[i] == std::string("-v")) {
       inery::cdt::environment::exec_subprogram("clang-7", {"-v"});
       return 0;
     }
   }

   cl::SetVersionPrinter([](llvm::raw_ostream& os) {
         os << COMPILER_NAME << " version " << "1.7.0" << "\n";
   });
   cl::ParseCommandLineOptions(argc, argv, std::string(COMPILER_NAME)+" (Inery C++ -> WebAssembly compiler)");
   ...
   try {
      for (auto input : opts.inputs) {
         ...
         if (!inery::cdt::environment::exec_subprogram("clang-7", new_opts)) {
   ...
   if (opts.link) {
      ...
      if (!inery::cdt::environment::exec_subprogram("inery-ld", new_opts)) {
         for (auto input : outputs) {
...
// need to generate dSYM for mac OSX
      ...
      if (opts.native) {
         if (opts.debug) {
            if (!inery::cdt::environment::exec_subprogram("dsymutil", std::vector<std::string>{opts.output_fn}, true)) {
               return -1;
            }
         }
      }
