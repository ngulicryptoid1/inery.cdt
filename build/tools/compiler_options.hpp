#pragma once
#include <inery/utils.hpp>
#include <inery/whereami/whereami.hpp>
#include <vector>
#include <string>
...
#ifdef ONLY_LD
#define LD_CAT IneryLdToolCategory
#else
#define LD_CAT IneryCompilerToolCategory
#endif

#ifndef ONLY_LD
static llvm::cl::OptionCategory IneryCompilerToolCategory("compiler options");
#else
static llvm::cl::OptionCategory IneryLdToolCategory("ld options");
#endif
...
static cl::opt<bool> use_old_malloc_opt(
    ...
    cl::Hidden,
    cl::cat(LD_CAT));
static cl::opt<std::string> inery_imports_opt(
    "inery-imports",
    cl::desc("Set the file for inery.imports"),
    cl::Hidden,
    cl::cat(LD_CAT));
static cl::opt<std::string> pp_path_opt(
    "inery-pp-dir",
    cl::desc("Set the directory for inery.pp"),
    cl::Hidden,
    cl::cat(LD_CAT));
...
static cl::opt<bool> llvm_ir_opt(
    ...
    cl::Hidden,
    cl::cat(IneryCompilerToolCategory));
...
static void GetCompDefaults(std::vector<std::string>& copts) {
   const char* inery_apply_suff = ".so";
   std::string apply_lib;
   // add the define for whether this is compiling with CDT and version macros
   copts.emplace_back("-D__inery_cdt__");
   copts.emplace_back(std::string("-D__inery_cdt_major__=")+"1");
   copts.emplace_back(std::string("-D__inery_cdt_minor__=")+"7");
   copts.emplace_back(std::string("-D__inery_cdt_patchlevel__=")+"0");
   if (!fnative_opt) {
      ...
      if (!fasm_opt) {
         ...
         copts.emplace_back("-D__inery_cdt_native__");
         copts.emplace_back("-DINERY_NATIVE");
   ...
   if (!fasm_opt) {
      ...
      if (!fnative_opt) {
         ...
         copts.emplace_back("-Xclang");
         if (llvm::sys::fs::exists(inery::cdt::whereami::where()+"/LLVMIneryApply"+inery_apply_suff))
            copts.emplace_back(inery::cdt::whereami::where()+"/LLVMIneryApply"+inery_apply_suff);
         else if (llvm::sys::fs::exists(inery::cdt::whereami::where()+"/../lib/LLVMIneryApply"+inery_apply_suff))
            copts.emplace_back(inery::cdt::whereami::where()+"/../lib/LLVMIneryApply"+inery_apply_suff);
      }
      if (false && fnative_opt) {
         copts.emplace_back("-Xclang");
         copts.emplace_back("-load");
         copts.emplace_back("-Xclang");
         if (llvm::sys::fs::exists(inery::cdt::whereami::where()+"/LLVMInerySoftfloat"+inery_apply_suff))
            copts.emplace_back(inery::cdt::whereami::where()+"/LLVMInerySoftfloat"+inery_apply_suff);
         else if (llvm::sys::fs::exists(inery::cdt::whereami::where()+"/../lib/LLVMInerySoftfloat"+inery_apply_suff))
            copts.emplace_back(inery::cdt::whereami::where()+"/../lib/LLVMInerySoftfloat"+inery_apply_suff);
      }
      if (llvm::sys::fs::exists(inery::cdt::whereami::where()+"/inery_plugin"+inery_apply_suff))
         copts.emplace_back("-fplugin="+inery::cdt::whereami::where()+"/inery_plugin"+inery_apply_suff);
      else if (llvm::sys::fs::exists(inery::cdt::whereami::where()+"/../lib/inery_plugin"+inery_apply_suff))
         copts.emplace_back("-fplugin="+inery::cdt::whereami::where()+"/../lib/inery_plugin"+inery_apply_suff);
   }
}
...
static void GetLdDefaults(std::vector<std::string>& ldopts) {
   if (!fnative_opt) {
      ...
      ldopts.emplace_back("-lc++ -lc -linery");
      if (use_old_malloc_opt)
         ldopts.emplace_back("-linery_malloc");
      else
         ldopts.emplace_back("-linery_dsm");

      if (use_rt_opt || fquery_opt || fquery_server_opt || fquery_client_opt)
         ldopts.emplace_back("-lrt -lsf");
      if (fquery_opt || fquery_server_opt || fquery_client_opt)
         ldopts.emplace_back("-linery_cmem");
      ...
      ldopts.emplace_back("-lnative_c++ -lnative_c -lnative_inery -lnative -lnative_rt");
   }
}
#endif
...
static Options CreateOptions(bool add_defaults=true) {
   ...
   if (!pp_path_opt.empty())
      ...
      pp_dir = inery::cdt::whereami::where();
...
#ifdef ONLY_LD
   ldopts.emplace_back("-L"+inery::cdt::whereami::where()+"/../lib");
#ifndef __APPLE__
   ldopts.emplace_back("-L"+inery::cdt::whereami::where()+"/../lib64");
#endif
#endif
#ifndef ONLY_LD
   ...
   if (!sysroot_opt.empty()) {
      ...
      // only allow capi for native builds and for inery-cc
      if (fnative_opt) {
        copts.emplace_back("-I"+sysroot_opt+"/include/inerylib/capi");
        copts.emplace_back("-I"+sysroot_opt+"/include/inerylib/native");
      }
#ifndef CPP_COMP
      copts.emplace_back("-I"+sysroot_opt+"/include/inerylib/capi");
#endif
      copts.emplace_back("-I"+sysroot_opt+"/include/inerylib/core");
      copts.emplace_back("-I"+sysroot_opt+"/include/inerylib/contracts");
      ...
      copts.emplace_back("-I"+inery::cdt::whereami::where()+"/../include/libcxx");
      copts.emplace_back("-I"+inery::cdt::whereami::where()+"/../include/libc");
      copts.emplace_back("-I"+inery::cdt::whereami::where()+"/../include");
      copts.emplace_back("--sysroot="+inery::cdt::whereami::where()+"/../");
      agopts.emplace_back("-I"+inery::cdt::whereami::where()+"/../include/libcxx");
      agopts.emplace_back("-I"+inery::cdt::whereami::where()+"/../include/libc");
      agopts.emplace_back("-I"+inery::cdt::whereami::where()+"/../include");
      agopts.emplace_back("--sysroot="+inery::cdt::whereami::where()+"/../");
      ldopts.emplace_back("-L"+inery::cdt::whereami::where()+"/../lib");

      if (fnative_opt) {
        copts.emplace_back("-I"+inery::cdt::whereami::where()+"/../include/inerylib/capi");
        copts.emplace_back("-I"+inery::cdt::whereami::where()+"/../include/inerylib/native");
      }
#ifndef CPP_COMP
      copts.emplace_back("-I"+inery::cdt::whereami::where()+"/../include/inerylib/capi");
#endif
      copts.emplace_back("-I"+inery::cdt::whereami::where()+"/../include/inerylib/core");
      copts.emplace_back("-I"+inery::cdt::whereami::where()+"/../include/inerylib/contracts");
   ...
   if (!fnative_opt) {
#ifndef ONLY_LD
      if (!inery_imports_opt.empty()) {
         ldopts.emplace_back("-inery-imports="+inery_imports_opt);
      }
#else
      if (!inery_imports_opt.empty()) {
         ldopts.emplace_back("--allow-undefined-file="+inery_imports_opt);
      }
      else {
         ldopts.emplace_back("--allow-undefined-file="+inery::cdt::whereami::where()+"/../inery.imports");
      }
#endif
   }
