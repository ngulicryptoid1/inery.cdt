...
#include <inery/gen.hpp>
#include <inery/whereami/whereami.hpp>
#include <inery/abi.hpp>
...
using namespace inery::cdt;
using jsoncons::json;
using jsoncons::ojson;

struct abigen_exception : public std::exception {
   virtual const char* what() const throw() {
      return "inery.abigen fatal error";
   }
} abigen_ex;


DeclarationMatcher function_decl_matcher = cxxMethodDecl().bind("inery_abis");
DeclarationMatcher record_decl_matcher   = cxxRecordDecl().bind("inery_abis");
DeclarationMatcher typedef_decl_matcher  = typedefDecl().bind("inery_abis");
auto               class_tmp_matcher     = classTemplateSpecializationDecl().bind("inery_abis");
...
class abigen : public generation_utils {
   ...
   void add_action( const clang::CXXRecordDecl* decl ) {
      abi_action ret;
      auto action_name = decl->getIneryActionAttr()->getName();
      ...
      if (action_name.empty()) {
         try {
            ...
            std::cout << "Error, name <" <<decl->getName().str() << "> is an invalid INERY name.\n";
            throw;
         }
         ...
         try {
            ...
            std::cout << "Error, name <" << action_name.str() << "> is an invalid INERY name.\n";
            throw;
         }
   ...
   void add_action( const clang::CXXMethodDecl* decl ) {
      ...
      if (action_name.empty()) {
         try {
            ...
            std::cout << "Error, name <" <<decl->getNameAsString() << "> is an invalid INERY name.\n";
         }
   ...
   void add_table( const clang::CXXRecordDecl* decl ) {
      ...
      auto table_name = decl->getIneryTableAttr()->getName();
      if (!table_name.empty()) {
         try {
   ...
   void add_table( uint64_t name, const clang::CXXRecordDecl* decl ) {
      if (!(decl->isIneryTable() && abigen::is_inery_contract(decl, get_contract_name())))
         return;
      abi_table t;
      t.type = decl->getNameAsString();
   ...
   std::string generate_json_comment() {
      std::stringstream ss;
      ss << "This file was generated with inery-abigen.";
      ss << " DO NOT EDIT";
      return ss.str();
   }
...
class IneryMethodMatcher : public MatchFinder::MatchCallback {
   public:
      virtual void run( const MatchFinder::MatchResult& res ) {
         if (const clang::CXXMethodDecl* decl = res.Nodes.getNodeAs<clang::CXXMethodDecl>("inery_abis")->getCanonicalDecl()) {
            abi abi;
            if (decl->isIneryAction() && abigen::is_inery_contract(decl, get_abigen_ref().get_contract_name())) {
...
class IneryRecordMatcher : public MatchFinder::MatchCallback {
   public:
      bool has_added_clauses = false;
      virtual void run( const MatchFinder::MatchResult& res ) {
         if (const clang::CXXRecordDecl* decl = res.Nodes.getNodeAs<clang::CXXRecordDecl>("inery_abis")) {
            ...
            if (decl->isIneryTable() && abigen::is_inery_contract(decl, get_abigen_ref().get_contract_name())) {
               get_abigen_ref().add_struct(decl);
         ...
         if (const clang::ClassTemplateSpecializationDecl* decl = res.Nodes.getNodeAs<clang::ClassTemplateSpecializationDecl>("inery_abis")) {
...
int main(int argc, const char **argv) {
   ...
   cl::SetVersionPrinter([](llvm::raw_ostream& os) {
        os << "inery-abigen version " << "1.7.0" << "\n";
  });
   cl::OptionCategory cat("inery-abigen", "generates an abi from C++ project input");
   ...
   options.push_back(std::string("-I")+inery::cdt::whereami::where()+"/../include/libcxx");
   options.push_back(std::string("-I")+inery::cdt::whereami::where()+"/../include/libc");
   options.push_back(std::string("-I")+inery::cdt::whereami::where()+"/../include");
   options.push_back(std::string("-I")+inery::cdt::whereami::where()+"/../../../../../libraries/libc++/libcxx/include");
   options.push_back(std::string("-I")+inery::cdt::whereami::where()+"/../../../../../libraries/libc/musl/include");
   options.push_back(std::string("-I")+inery::cdt::whereami::where()+"/../../../../../libraries");
   options.push_back(std::string("-I")+inery::cdt::whereami::where()+"/../../../../../libraries/boost/include");
   ...
   IneryMethodMatcher inery_method_matcher;
   IneryRecordMatcher inery_record_matcher;
   MatchFinder finder;
   finder.addMatcher(function_decl_matcher, &inery_method_matcher);
   finder.addMatcher(record_decl_matcher, &inery_record_matcher);
   finder.addMatcher(class_tmp_matcher, &inery_record_matcher);

   llvm::errs() << "Warning, this tool is deprecated.  Only use inery-cpp in the future." << '\n';
   int tool_run = -1;
   try {
